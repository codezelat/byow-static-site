...
export default function PopupPage({ onClose }: PopupPageProps) {
  ...
  return (
    <div className="fixed inset-0 bg-[#040010] bg-opacity-50 backdrop-blur-md flex items-center justify-center z-50">
        <div 
         className="bg-[#060116] rounded-4xl shadow-xl w-[95%] md:w-[1191px] h-[95%] md:h-[984px] mx-4 p-6 max-h-[90vh] overflow-y-auto relative"
        style={{
          border: '1px solid',
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-6 justify-center">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm mb-2 text-start">Last Name*</label>
              <input 
                type="text" 
                name="lastName" 
                required
          ...
          <div>
            ...
            <input 
              ...
              name="business" 
              required
              className="w-full bg-[#18171A] text-white px-4 py-3 rounded-[64px] border border-transparent focus:border-[#8133F1] focus:outline-none"
              placeholder="Aronian"
            />
          </div>
        ...
        <div className="mt-6 flex flex-col items-start space-y-4 relative">
          ...
          <div className="absolute top-0 right-0 flex gap-4 text-white bg-[#18171A] items-center w-[288px] h-[50px] rounded-[64px] px-6 py-3">
